...
bradleyTerry <- function(data, maxIter, ...) {
  ...
  if(class(data) == "list"){
    ...
    obj <- new("BradleyTerry", parameters = c(params))
  }else{
...
#' @param model object of the class BradleyTerry.
#' @param data population whoose probabilities are desired to know.
calc.prob <- function(model, data){
  if(class(model) != "BradleyTerry"){
    stop("The model must be of class BradleyTerry")
  }else{
    result <- list()
